...
def load_data():
    data = {
        '交易时间': [
            ...
            '2007-11-30', '2007-12-28', '2008-01-31', '2008-02-29', '2008-03-31',
            '2008-04-30', '2008-05-30', '2008-06-30', '2008-07-31', '2008-08-29',
            '2008-09-26', '2008-10-31', '2008-11-28', '2008-12-31', '2009-01-23',
            '2009-02-27', '2009-03-31', '2009-04-30', '2009-05-27'
        ],
        '收盘价': [
            2786.33, 2881.07, 3183.98, 3841.27, 4109.65,
            3820.70, 4471.03, 5218.83, 5552.30, 5954.77,
            4871.78, 5261.56, 4383.39, 4348.54, 3472.71,
            3693.11, 3433.35, 2736.10, 2775.72, 2397.37,
            2293.78, 1728.79, 1871.16, 1820.81, 1990.66,
            2082.85, 2373.21, 2477.57, 2632.93
        ],
        '涨跌幅%': [
            4.1436, 3.4002, 10.5138, 20.6437, 6.9868,
            -7.031, 17.0212, 16.7255, 6.3897, 7.2487,
            -18.1869, 8.0008, -16.6903, -0.795, -20.1408,
            6.3466, -7.0336, -20.3082, 1.448, -13.6307,
            -4.321, -24.6314, 8.2352, -2.6908, 9.3283,
            4.6311, 13.9405, 4.3974, 6.2707
        ]
    }
...
def generate_comment(action, next_row):
    ...
    if action == "持有":
        if change > 0:
            msgs = [
                f"🎯 明智之举！{month}上涨{change:.2f}%，朋友们都羡慕你！",
                f"🚀 股神附体！{month}大涨{change:.2f}%，你已成传说！",
                f"💰 精准判断！{month}涨了{change:.2f}%，加鸡腿！",
                f"🏆 太强了！{month}上涨{change:.2f}%，财富神话！"
            ]
        else:
            msgs = [
                f"😅 有点可惜！{month}跌了{change:.2f}%，好戏在后头！",
        ...
        if change > 0:
            msgs = [
                f"😱 卖飞了！{month}大涨{change:.2f}%，朋友们笑你太急！",
            ...
            msgs = [
                f"🛡️ 避险成功！{month}大跌{change:.2f}%，躲过一劫！",
                f"🧠 高手风范！{month}跌了{change:.2f}%，空仓明智！",
                f"💡 未雨绸缪！{month}下跌{change:.2f}%，判断精准！",
                f"🌟 大师级别！{month}跌了{change:.2f}%，满分操作！"
            ]
    else:  # 买入
        if change > 0:
            msgs = [
                f"🚀 抄底成功！{month}大涨{change:.2f}%，精准入场！",
                f"💰 眼光独到！{month}涨了{change:.2f}%，低位建仓！",
                f"🎯 完美时机！{month}上涨{change:.2f}%，高手！"
            ]
        else:
            msgs = [
                f"😅 买早了！{month}跌了{change:.2f}%，但机会是等出来的！",
...
def main():
    ...
    with col1:
        ...
        if st.session_state.step > 0:
            hist = df.iloc[:st.session_state.step]
        ...
        if st.session_state.step < len(df):
            future = df.iloc[st.session_state.step:]
            fig.add_trace(go.Scatter(
                x=future['交易时间'],
                y=future['收盘价'],
                mode='lines',
    ...
    with col2:
        ...
        if st.session_state.game_over:
            ...
            if final_rate > 50:
                st.success("🏆 股神！你战胜了市场！")
            elif final_rate > 20:
                st.success("🌟 优秀！投资眼光不错！")
            elif final_rate > 0:
                st.info("👍 稳健！保住了本金！")
            elif final_rate > -20:
                st.warning("😅 有点遗憾，市场太残酷了！")
            else:
                st.error("💀 被市场教育了，下次再来！")
